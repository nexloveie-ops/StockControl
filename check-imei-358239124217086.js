...
async function checkIMEI() {
  try {
    ...
    const imei = '358239124217086';
    console.log(`=== 查询IMEI: ${imei} ===\n`);

    // 1. 检查 MerchantInventories
    console.log('1. 检查 MerchantInventories:');
    ...
    if (merchantInventory.length > 0) {
      merchantInventory.forEach(item => {
        ...
        console.log(`   - 来源ID: ${item.sourceTransferId || item.sourceOrderId || '无'}`);
    ...
    // 2. 检查 WarehouseOrders
    console.log('2. 检查 WarehouseOrders:');
    ...
    // 3. 检查 InventoryTransfers
    console.log('3. 检查 InventoryTransfers:');
    ...
    // 4. 检查 MerchantSales
    console.log('4. 检查 MerchantSales:');
    ...
    // 5. 检查 AdminInventories
    console.log('5. 检查 AdminInventories:');
    ...
    if (adminInventory.length > 0) {
      adminInventory.forEach(item => {
        ...
        console.log(`   - 产品: ${item.productName}`);
        console.log(`   - 序列号: ${item.serialNumber}`);
        console.log(`   - 店铺组: ${item.storeGroup}`);
        console.log(`   - 状态: ${item.status}`);
        console.log('');
      });
    } else {
    ...
    // 6. 检查 PurchaseInvoices
    console.log('6. 检查 PurchaseInvoices:');
    const purchaseInvoices = await mongoose.connection.db.collection('purchaseinvoices')
      .find({ 'items.serialNumbers': imei })
      .toArray();
