/**
 * 检查序列号 111555 的销售和退款记录
 */

const mongoose = require('mongoose');
require('dotenv').config();

async function checkSale111555() {
  try {
    ...
    if (refundedSale) {
      ...
      if (saleItem) {
        ...
        if (saleItem.refundCondition) {
          console.log(`✅ 退款时选择的成色: ${saleItem.refundCondition}`);
          if (saleItem.refundCondition !== displayCondition) {
            console.log(`⚠️  但时间线显示的是: ${displayCondition}`);
          }
        } else {
          console.log('⚠️  退款时没有保存 refundCondition 字段');
        }
      }
    } else {
